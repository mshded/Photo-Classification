...
def save_positive_images(df: pd.DataFrame, output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    kept_df = df[df["baseline_keep"]].copy()
...
def run_pipeline_for_url(url: str, output_dir: str, raw_dir: str | None = None) -> Dict:
    ...
    if tmp_df.empty:
        ...
        (page_output_dir / "baseline_results.csv").write_text("", encoding="utf-8")
        summary = summarize_baseline_results(tmp_df)
        summary.update({"url": url, "page_id": page_id, "results_csv": str(page_output_dir / "baseline_results.csv")})
        return summary
    ...
    save_positive_images(df, page_output_dir)

    summary = summarize_baseline_results(df)
    summary.update({"url": url, "page_id": page_id, "results_csv": str(results_csv)})
    return summary
